...
/**
 * Directory entries a deleted package's leftover can still contain.
 *
 * Every one of these is regenerated by an install or a build, so removing them
 * loses nothing that cannot be recreated.
 */
const HUSK_ONLY_ENTRIES = new Set([
  'node_modules',
  'dist',
  'lib',
  'build',
...
/** Extensions that mean "someone wrote source code here — do not touch it". */
const SOURCE_FILE_RE =
  /\.(ts|tsx|mts|cts|js|jsx|mjs|cjs|json|jsonc|vue|svelte|rs|toml|ya?ml|md|py|go|sh|ps1|css|scss|html)$/i
...
export class RuntimeSource {
  ...
  private async buildAll(progress: ProgressFn): Promise<void> {
    ...
    // Directory husks from packages this revision deleted would be picked up
    // by tsdown's workspace glob and abort the whole build. Clear them first —
    // see pruneOrphanWorkspaceDirs.
    this.pruneOrphanWorkspaceDirs()
  ...
  /**
   * Delete husk directories left behind by packages a newer revision removed.
   *
   * deepseek-harness prunes packages between releases — rc.2 dropped
   * @deepseek-ai/dsh-util-values and 28 others. `git reset --hard` deletes the
   * tracked files but NOT the directory, because the directory still holds
   * untracked content, and pnpm has put a node_modules/ inside every workspace
   * package. What is left looks exactly like a package to any glob that matches
   * on path:
   *
   *   tsdown workspace: ['vendor/<pkg>', 'packages/<group>/<pkg>', 'apps/cli']
   *
   * tsdown then tries to build it, finds no lib/types/*.js to bundle, and
   * aborts the ENTIRE build — including the 220+ packages that are healthy:
   *
   *   [@deepseek-ai/dsh-root] Cannot find entry: ["lib/types/{index,invariant,startup}.js"]
   *
   * The package name in that message is a red herring, and it is why this needs
   * a comment: tsdown falls back to the root package's name when a directory has
   * no package.json to read one from, so a husk's failure is reported against
   * the monorepo anchor. The message sends you to inspect the root package,
   * which is not what is broken.
   *
   * This is not local corruption. Anyone who updates across a revision that
   * deletes a package hits it, so the cleanup belongs here rather than in a
   * one-time local fixup.
   *
   * @returns the number of directories removed.
   */
  private pruneOrphanWorkspaceDirs(): number {
    // Two shapes: the grouped packages/<group>/<pkg> and the flat vendor/<pkg>
    // and apps/<pkg>.
    const specs: Array<{ dir: string; depth: 1 | 2 }> = [
    ...
    for (const spec of specs) {
      const base = join(this.dir, spec.dir)
      if (!existsSync(base)) continue
      for (const dir of this.workspacePackageDirs(base, spec.depth)) {
        // A directory with a manifest is a real package, whatever else is wrong.
        if (existsSync(join(dir, 'package.json'))) continue
        if (!this.isDeletedPackageHusk(dir)) continue
        try {
          rmSync(dir, { recursive: true, force: true })
          removed.push(relative(this.dir, dir).split(sep).join('/'))
        } catch (err) {
          log(
            'launcher',
            `pruneOrphanWorkspaceDirs: cannot remove ${relative(this.dir, dir)}: ${(err as Error).message.slice(0, 120)}`
          )
        }
      }
    }
    if (removed.length > 0) {
      log(
        'launcher',
        `pruneOrphanWorkspaceDirs: removed ${String(removed.length)} deleted-package leftover(s): ${removed.slice(0, 6).join(', ')}`
      )
    }
  ...
  /**
   * Candidate package directories under base, at the workspace's nesting depth.
   */
  private workspacePackageDirs(base: string, depth: 1 | 2): string[] {
    const out: string[] = []
    const top = this.subdirs(base)
    for (const dir of top) {
      if (depth === 1) {
        out.push(dir)
        continue
      }
      for (const nested of this.subdirs(dir)) out.push(nested)
    }
    return out
  }

  /** Immediate child directories of base, skipping ones that cannot be packages. */
  private subdirs(base: string): string[] {
    let entries: Dirent[]
    try {
  ...
  /**
   * Confirm a directory is a removed package's leftover rather than a real one.
   *
   * Deleting is irreversible, so this stays conservative: a false positive
   * destroys files. It takes two independent confirmations — that the revision
   * no longer tracks anything here, and that nothing source-like remains — and
   * refuses to remove anything unless both hold.
   */
  private isDeletedPackageHusk(dir: string): boolean {
    // Someone's uncommitted source must never be collateral damage.
    if (this.containsSourceCode(dir)) return false

    // Preferred: git knows whether this revision tracks anything here. A
    // package directory with zero tracked files is one this revision deleted.
    // `git ls-files` wants a slash-separated path relative to the repo root.
    const rel = relative(this.dir, dir).split(sep).join('/')
    if (this.gitTrackedFileCount(rel) === 0) return true

    // No usable git answer (ZIP-copied workspace, git missing, or the query
    // failed): fall back to inspecting contents. A husk is a directory that
    // only an install or a build has ever written to.
    let entries: string[]
    try {
    ...
    return entries.length > 0 && entries.every((name) => HUSK_ONLY_ENTRIES.has(name))
  }

  /**
   * Look for source files under dir, ignoring installed dependencies.
   *
   * Depth-limited: a husk is shallow, so there is no reason to walk a deep
   * tree to answer this.
   */
  private containsSourceCode(dir: string, depth = 3): boolean {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (this.containsSourceCode(join(dir, entry.name), depth - 1)) return true
        continue
      }
      if (SOURCE_FILE_RE.test(entry.name)) return true
    }
    return false
  }

  /**
   * How many files git tracks under a path.
   *
   * Returns undefined when git cannot answer — not a repository, git not
   * installed, or the command failed. Callers must treat undefined as
   * "inconclusive" and fall back to another check rather than as "zero".
   */
  private gitTrackedFileCount(relPath: string): number | undefined {
    try {
